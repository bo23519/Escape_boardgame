 ...
 * into a valid XML string that is then unmarshalled into an
 * EscapeGameInitializer file.
 * 
 * MODIFIABLE: YES MOVEABLE: NO REQUIRED: YES
 * 
 * >>>THIS IS AN EXAMPLE STARTER FILE<<< You must change this class to be able
 * to get the data from the configurtion file and implement the
 * makeGameManager() method. You may not change the signature of that method or
 * the constructor for this class. You can change the file any other way that
 * you need to.
 * 
 * You do not have to use JAXB to create XML. You might use GSON if you are more
 * familiar with it. You also don't have to use the EscapeGameInitializer object
 * if you have a way that better suits your design and capabilities. Don't go
 * down a rathole, however, in order to use something different. This
 * implementation works and will not take much time to modify the
 * EscapeGameInitializer to create your game instance.
 */
public class EscapeGameBuilder {
	private final EscapeGameInitializer gameInitializer;

	/**
	 * The constructor takes a file that points to the Escape game configuration
	 * file. It should get the necessary information to be ready to create the game
	 * manager specified by the configuration file and other configuration files
	 * that it links to.
	 * 
	...
	/**
	 * Take the .egc file contents and turn it into XML. If you want to use JSON,
	 * you should change the name of this method and initizlize the
	 * EscapeConfigurator with this code: EscapeConfigurator configurator = new
	 ...
	 * instance. For this example, you would use the gameInitializer object to get
	 * all of the information you need to create your game.
	 * 
	 * @return the game instance
	 */
	public EscapeGameManager makeGameManager() {
		EscapeGameManagerImpl<Coordinate> gameManager = new EscapeGameManagerImpl<Coordinate>();
		gameManager.initialize(gameInitializer);
		return gameManager;
	}
}
